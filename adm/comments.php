...
<a href="pending_comments.php" style="width: 200px;height:40px;" class="btn btn-success">Pending comments</a>
...
if(isset($_GET['search'])){
    ...
    $select_file = mysqli_query($conn, "SELECT * FROM `commenttable` WHERE approved='yes' AND  blogid='$blogid' ORDER BY id DESC") or die('query failed');
    if(mysqli_num_rows($select_file) > 0){
        $sno=1;
       while($fetch_products = mysqli_fetch_assoc($select_file)){
?>
...
<?php
        ...
        echo "<tr class='text-center'><td colspan=6>No Comments to Show</td></tr>";
       }
}
    ...
    $select_file = mysqli_query($conn, "SELECT * FROM `commenttable` WHERE approved='yes' ORDER BY id DESC") or die('query failed');
    if(mysqli_num_rows($select_file) > 0){
        $sno=1;
       while($fetch_products = mysqli_fetch_assoc($select_file)){
